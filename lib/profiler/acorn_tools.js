...
exports.dump = function(o, t, r, iterations){
	...
	for(var k in o)if(o.hasOwnProperty(k)){
      ...
      if (iterations >= 2000) {
        throw new Error();
			...
			s += exports.dump(v, t + ' ', r, iterations++)
		}
		else s += v
	}
	s += '\n'+t.slice(1) + (a?']':'}')
...
function walkUp(p, o){
	while(p){
		var f = o[(p.node || p).type]
		if(f && f((p.node || p), p)) break
		p = p.up
	}
}
...
var sTab = {
	...
	TryStatement:         function(n){
		return 'try '+sExp(n.block)+sSep+sBlk(n.handler)+sSep+
